...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    val acc_plus = if p(elem) then acc.incl(elem) else acc
    left.filterAcc(p, right.filterAcc(p, acc_plus))

  override def union(that: TweetSet): TweetSet =
    left.union(right.union(that.incl(elem)))

  override def mostRetweeted: Tweet = {
    def max(a : Tweet, b : Tweet) : Tweet = if a.retweets < b.retweets then b else a

    if left.isEmpty && right.isEmpty then elem
    else if left.isEmpty then max(elem, right.mostRetweeted)
    else if right.isEmpty then max(left.mostRetweeted, elem)
